...
def _build_category_text(tpl, category, articles):
    ...
    max_arts = tpl.get('max_articles_per_category', 5)
    max_len = tpl.get('max_title_length', 60)
...
def main():
    ...
    pdf = sys.argv[1]
    msg = sys.argv[2] if len(sys.argv) > 2 else '本日のAWSブログ要約です'
