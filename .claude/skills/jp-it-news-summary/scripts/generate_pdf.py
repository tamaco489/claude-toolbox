...
# デフォルトテンプレート
DEFAULT_TPL = {
    "title": "国内ITニュース要約レポート",
    "date_format": "作成日: {date}",
    "category_format": "■ {category}",
    "source_format": "出典: {source} | {url}",
    "no_articles_message": "取得した記事はありません。",
    "styles": {
        "title": {"font_size": 18, "color": "#1a1a1a", "leading": 24, "space_after": 12},
        "date": {"font_size": 10, "color": "#6b7280", "leading": 12, "space_after": 20},
        "category_header": {"font_size": 14, "color": "#059669", "leading": 18, "space_before": 16, "space_after": 8},
        "article_title": {"font_size": 11, "color": "#1f2937", "leading": 14, "space_before": 8, "space_after": 2},
...
def _build_content(data, tpl, styles):
    ...
    # タイトル・日付
    content.append(Paragraph(tpl.get('title', '国内ITニュース要約レポート'), styles['JapaneseTitle']))
...
def main():
    ...
    out_path = os.path.join(out_dir, f'jp-it-news-{date}.pdf')
